...
const PatientSearch = () => {
  ...
  useEffect(() => {
    fetchDoctors();
  }, [specialty, treatmentType]);

  const fetchDoctors = async () => {
    try {
      const url = treatmentType !== 'All' ? `/doctors?treatment_type=${treatmentType}` : '/doctors';
    ...
    <MobileLayout title="MedCore Central">
      <div className="px-4 py-6">
        ...
        <div className="space-y-5 bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6">
          ...
          <div>
            <label className="block text-xs font-bold text-gray-700 uppercase mb-2 tracking-wider">Treatment Type</label>
            <div className="flex gap-2 flex-wrap">
              {['All', 'Allopathic', 'Homeopathic', 'Ayurvedic'].map(type => (
                <button 
                  key={type}
                  onClick={() => setTreatmentType(type)}
                  className={`flex-1 py-1.5 px-2 text-xs font-medium rounded-full whitespace-nowrap ${
                    treatmentType === type ? 'bg-blue-600 text-white' : 'bg-blue-50 text-blue-600'
                  }`}
                >
                  {type}
                </button>
